...
export function setup(ctx) {
  ...
  function buildNativeControls() {
    ...
    const maxTokensField = createField("Maximum tracker output tokens", "Allowed range: 400–8,192 tokens.");
  ...
  function renderStatus(status) {
    ...
    controls.maxTokens.set(status.config?.maxTokens ?? 4_096);
    controls.timeout.set(Math.round((status.config?.timeoutMs ?? 120_000) / 1_000));
    renderConnections();
