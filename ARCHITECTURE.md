...
Both CHAT_SWITCHED and rendered-message chat changes use the same reset function so a stale save completion cannot leave setupBusy set in another chat. Repeated saves on the same chat deliberately create fresh native pickers; HTTP exports retain the prepared JSON until replaced or the chat changes; each download button click creates a fresh temporary anchor.
