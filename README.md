...
## Save and load an initial setup (v1.4.2)
...
- If a profile card remains in its checking/no-engine fallback after updating, confirm Continuity Engine v1.4.2 and the card's current persistent-state companion are installed. Version 1.2.2 and later detect and update profile cards inside Lumiverse's open Shadow DOM HTML islands; no extra chat turn or manual click should be required.
