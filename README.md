...
- Profile-card lookup traverses open Shadow DOM HTML islands inside mounted message bubbles and applies live status directly to the card, because document-level selectors and styles do not cross a shadow boundary. A successful shadow-root lookup reports a positive `shadowCardCount` and a nonzero `profile_card_update.cardCount`.
