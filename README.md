...
## Save and load an initial setup (v1.4.1)
...
- On a remote HTTP connection or a browser without that API, export prepares the file and displays a clearly outlined **Download JSON File** button. Click it to start the download. Whether a Save As window appears then depends on browser download settings; otherwise check Downloads. You can rename and keep as many files as you like.
...
- If a profile card remains in its checking/no-engine fallback after updating, confirm Continuity Engine v1.4.1 and the card's current persistent-state companion are installed. Version 1.2.2 and later detect and update profile cards inside Lumiverse's open Shadow DOM HTML islands; no extra chat turn or manual click should be required.
